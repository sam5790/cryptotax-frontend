<template>
    <div class="mx-auto w-full md:p-7 md:my-8">
                    ...
                    <div class="relative inline-block text-left mt-1 w-full">
                        ...
                        <div v-if="openSelectExchange"
                            ...
                            <ul class="text-sm text-gray-700 max-h-60 overflow-y-auto">
                                <li v-for="exchange in futureTransactionsPagination?.exchanges" :key="exchange" @click="
                ...
                <div>
                    ...
                    <div class="relative inline-block text-left mt-1 w-full">
                        ...
                        <div v-if="openSelectCoin"
                            class="absolute right-0 top-12 bg-white rounded-lg w-full border border-gray-200 shadow-lg z-50 animate-fadeIn">
                            <ul class="text-sm text-gray-700 max-h-60 overflow-y-auto">
                                <li v-for="coin in futureTransactionsPagination?.coins" :key="coin" @click="
        ...
        <div class="hidden md:block">
            <table class="w-full text-left">
                        ...
                        <th class="py-2 px-4 border-b font-semibold relative">
                            ...
                            <div v-if="openExchangeFilter"
                                ...
                                <ul class="text-sm text-gray-700 max-h-48 overflow-y-auto"
                                    v-if="futureTransactionsPagination?.exchanges?.length > 0">
                                    <li v-for="exchange in futureTransactionsPagination.exchanges" :key="exchange"
                        ...
                        <th class="py-2 px-4 border-b font-semibold">
                            <div class="flex items-center gap-1">
                                ...
                                <div class="flex flex-col -space-y-2">
                                    ...
                                    <Icon name="mdi-menu-down" class="cursor-pointer text-xl"
                                        @click.stop="handleSort('pnl', 'desc')"
                ...
                <tbody v-else>
                    <tr v-for="(item, index) in futureTransactions" :key="index" class="border-b">
                        <td class="p-4">
                            {{ index + (futureTransactionsPagination.currentPage - 1) * 50 + 1 }}
                        </td>
                        <td class="p-4">
                            <div class="flex items-center gap-2">
                        ...
                        <td class="p-4 font-[Poppins]" :class="item?.pnl >0 ? 'text-green-500':item?.pnl <0 ? 'text-red-500':''">{{ item?.pnl }}</td>
        ...
        <div class="md:hidden flex flex-col space-y-3 p-2" v-if="loading || futureTransactions?.length">
            ...
            <template v-else>
                <div v-for="(item, index) in futureTransactions" :key="index"
                    ...
                    <div class="flex justify-between mb-1">
                        ...
                        <span class="font-[Poppins]" :class="item?.pnl >0 ? 'text-green-500':item?.pnl <0 ? 'text-red-500':''">{{ item?.pnl }}</span>
                    </div>
    ...
    <div class="flex justify-center gap-3 mt-6" v-if="!loading && futureTransactions?.length">
        <button @click="changePage(futureTransactionsPagination.currentPage - 1)"
            :disabled="futureTransactionsPagination.currentPage === 1"
            class="px-4 py-2 rounded bg-gray-200 disabled:opacity-40">
            Previous
        </button>

        <span class="px-4 py-2 font-[Poppins]">
            Page {{ futureTransactionsPagination?.currentPage }} of
            {{ futureTransactionsPagination?.totalPages }}
        </span>

        <button @click="changePage(futureTransactionsPagination.currentPage + 1)" :disabled="futureTransactionsPagination.currentPage ===
            futureTransactionsPagination.totalPages
            " class="px-4 py-2 rounded bg-gray-200 disabled:opacity-40">
            Next
        </button>
    </div>
    <div v-if="!loading && futureTransactions?.length === 0"
...
const { futureTransactions, futureTransactionsPagination } = storeToRefs(store);
...
const getFutureTransactionData = async () => {
    loading.value = true;
    const payload = {
        page: page.value,
        limit: 50,
    ...
    await getFutureTransactions(payload)
    loading.value = false;
}
...
const coinList = computed(() => {
    const uniqueCoins = futureTransactionsPagination.value?.coins;
...
watch([selectedExchange, selectedCoin, page, sortBy, sortOrder, selectedDate, transactionType, selectedType], async () => {
    await getFutureTransactionData()
});
onUnmounted(() => {
