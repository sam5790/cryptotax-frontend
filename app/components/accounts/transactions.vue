<template>
  <div>
    <div class="flex flex-col sm:flex-row justify-center items-center gap-6 p-5">
      <div class="shadow-2xl p-6 text-center rounded-2xl relative w-64 h-25">
        <div
          class="absolute -top-6 left-1/2 -translate-x-1/2 bg-white shadow-lg w-12 h-12 rounded-full flex items-center justify-center">
      ...
      <div class="shadow-2xl p-6 text-center rounded-2xl relative w-64 h-25 max-sm:mt-2">
        <div
          class="absolute -top-6 left-1/2 -translate-x-1/2 bg-white shadow-lg w-12 h-12 rounded-full flex items-center justify-center">
    ...
    <div class="container bg-white mx-auto w-full md:p-7 shadow-lg md:my-8" v-if="transactions.length > 0">
      <div class="flex flex-col sm:flex-row justify-end gap-3 p-4 md:p-7">
        ...
        <button class="rounded-full text-sm py-2 px-3 sm:px-4 bg-teal-500 text-white font-medium" @click="exportPnl">
          EXPORT PNL
        </button>
      </div>
        ...
        <table class="w-full text-left">
          ...
          <tbody>
            <tr v-for="(item, index) in transactions" :key="index" class="border-b">
              ...
              <td class="py-4 px-4">
                <span :class="item.type === 'buy' ? 'text-green-500' : 'text-red-500'">
                  {{ item.quantity }}
                </span>
              </td>
      ...
      <div class="md:hidden flex flex-col space-y-3 p-2">
        <div v-for="(item, index) in transactions" :key="index" class="border p-4 rounded-lg shadow-md bg-white">
          ...
          <div class="flex justify-between mb-1">
            <span>Quantity:</span>
            <span :class="item.type === 'buy' ? 'text-green-500' : 'text-red-500'">{{ item.quantity }}</span>
          </div>
          <div class="flex justify-between mb-1">
            <span>Amount:</span>
          ...
          <div class="flex justify-between">
            <span>TDS:</span>
            <span>{{ item.tds || 0 }}</span>
          </div>
        </div>
      </div>
    </div>
    <div
      class="fixed bottom-4 right-4 sm:bottom-8 sm:right-8 bg-white border border-teal-500 rounded-xl flex items-center px-4 sm:px-10 py-3 sm:py-5 shadow-md z-20">
