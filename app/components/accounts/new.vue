<template>
  <div class="md:p-10">
    <div class="flex flex-col justify-center items-center">
      <div
        ...
        <div class="absolute -top-[60px] left-1/2 -translate-x-1/2">
          <img src="/new-avatar.png" class="w-28 h-28 rounded-full bg-white shadow-[0_0_10px_0] shadow-[#254BD34D]" />
        </div>
        ...
        <div class="flex items-center justify-center gap-4 sm:gap-10 w-full">
          <div class="text-center sm:p-2">
            ...
            <div class="flex justify-center mt-2">
              <svg class="sm:w-6 sm:h-6 w-4 h-4" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
          ...
          <div class="text-center bg-[#37D35921] rounded-2xl sm:px-6 px-2 py-3">
            <Icon v-if="loading" name="mdi-loading" class="animate-spin mt-1 h-8 w-8" />
            <div v-else class="sm:text-3xl text-lg font-medium font-[Poppins]" :class="totalTransactions?.PnlSum > 0
              ? 'text-[#31B431]'
          ...
          <div class="text-center sm:p-2">
            ...
            <div class="flex justify-center mt-2">
              <Icon name="mdi:files" class="sm:w-6 w-4 sm:h-6 h-4 text-[#4AABAB]" />
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="relative flex items-center gap-2">
            ...
            <div v-if="open === item._id"
              ...
              <ul class="py-2 text-sm text-gray-700">
                <li @click="buttonclick" class="px-4 py-2 hover:bg-[#4aabab]/10 cursor-pointer flex items-center gap-1">
                  <Icon name="mdi-plus" class="text-[#4aabab]" /> Add Files
                </li>
                <li v-if="item?.fileUploadCount > 0"
                ...
                <li v-else class="px-4 py-2 hover:bg-[#4aabab]/10 cursor-pointer flex items-center gap-1"
                  @click="confirmDeleteAccount(item)">
                  <Icon name="mdi-delete" class="text-red-600" /> Delete Account
                </li>
                <li v-if="item?.fileUploadCount > 0"
                  class="px-4 py-2 hover:bg-[#4aabab]/10 cursor-pointer flex items-center gap-1"
                  @click="handleRecalculate(item?.exchange)">
                  <Icon name="mdi:cached" class="text-[#4AABAB] cursor-pointer hover:scale-105 active:scale-95">
                  </Icon> Recalculate
                </li>
              </ul>
            </div>
    ...
    <div v-if="confirm" class="fixed inset-0 bg-black/40 flex items-center justify-center p-2">
      <div class="bg-white rounded-xl shadow-lg p-4 w-full max-w-xs text-center">
        <p class="text-lg font-medium mb-3">Select the files</p>
        <div class="max-h-40 overflow-y-auto mb-4 text-left border border-gray-100 rounded-lg p-1"
          v-if="totalFiles.length > 0">
          <div v-for="(file, index) in totalFiles" :key="index"
            ...
            <div class="flex items-center gap-2 overflow-hidden">
              <Icon name="mdi:file-excel" class="text-3xl text-[#4aabab] shrink-0" />
              <span class="text-sm text-gray-600 truncate" :title="file.fileName || file.name">
                {{ file.fileName || file.name || `File ${index + 1}` }}
              </span>
            </div>
            <input @change="handelCheckedFile($event, file)" type="checkbox"
        ...
        <div class="flex justify-center gap-3">
          ...
          <button :disabled="deleteLoading" @click="deleteExchangeFiles(totalFiles?.[0]?.exchange)"
    ...
    <div v-if="deleteAccountConfirm" class="fixed inset-0 bg-black/40 flex items-center justify-center p-2">
      <div class="bg-white rounded-xl shadow-lg p-4 w-full max-w-md text-center">
        <p class="text-lg font-medium mb-3">Delete Account</p>
        <div class="mb-4">
          Are you sure you want to delete
          <span class="font-medium capitalize text-[#4AABAB]">"{{ selectedAccount?.exchange }}"</span>
          account?
        </div>

        <div class="flex justify-center gap-3">
          <button @click="deleteAccountConfirm = false"
            class="px-4 py-1 rounded-md border border-gray-300 hover:bg-gray-100">
            Cancel
          </button>

          <button @click="deleteExchange(selectedAccount?._id)"
