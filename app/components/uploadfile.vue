<template>
  <div class="p-4">
    ...
    <div class="text-center mt-6">
      ...
      <p v-if="fileName.length > 0" class="flex items-center justify-center gap-2 mt-2 text-green-600">
        {{ fileName }}
      </p>

      <Uploader ref="uploaderRef" :title="account.title"
        :uploadEndpoint="account.title === 'Coin DCX' ? 'coindcx' : 'wasirx'" @uploadSuccess="handleUploadSuccess" />
      <button @click="triggerUploaderUpload" class="mt-5 px-5 py-3 border border-teal-600 text-teal-600 rounded-lg ">
...
const triggerUploaderUpload = () => {
  console.log(uploaderRef.value.uploadFile)
