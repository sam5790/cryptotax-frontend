<template>
  ...
  >
    <div
      class="bg-white p-6 md:p-10 rounded-2xl shadow-2xl max-h-full overflow-y-auto w-[95%] max-w-7xl relative"
      @click.stop
    >
      <button
        @click="$emit('close')"
      ...
      <div class="flex justify-center mt-6 md:mt-10">
        <div class="bg-white rounded-full p-1 flex gap-1 border">
          <button
            @click="tab = 'auto'"
            :class="tab === 'auto' && 'bg-[#4aabab] text-white'"
          ...
          <button
            @click="tab = 'custom'"
            :class="tab === 'custom' && 'bg-[#4aabab] text-white'"
            class="px-4 py-1 text-sm font-medium rounded-full"
          >
            Custom
          </button>
      ...
      <div class="mt-6 md:mt-10 flex items-center justify-center">
        <div class="relative w-full max-w-[70%]">
          ...
          <input
            v-model="searchText"
            type="text"
            placeholder="e.g. Binance, WazirX, Ethereum"
      ...
      <div class="border rounded-2xl mt-6 p-5 w-full">
        <div class="flex flex-col lg:flex-row gap-6">
          ...
          <div v-if="filteredList?.length"
            class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 w-full"
          >
            <div
              class="border flex items-center p-4 cursor-pointer max-h-[70px]"
          ...
          <div v-else class="flex justify-center items-center w-full">No result found!</div>
        </div>
      </div>
    </div>
  </div>
...
const list = ref([
  ...
  {
    icon: "/icons/mudrex.png",
    title: "Murdex",
    type: "Exchange",
  },
  {
    icon: "/icons/coinswitch.png",
    title: "Coinswitch",
    type: "Exchange",
  },
  {
...
const filteredList = computed(() => {
  ...
  return list.value.filter((item) => {
    ...
    // convert selected (e.g. "Exchanges") to singular to match item.type (e.g. "Exchange")
    const sel = selected.value.toLowerCase();
    const selSingular = sel.endsWith("s") ? sel.slice(0, -1) : sel;

    return (
      matchesSearch && item.type.toLowerCase() === selSingular
    );
  });
});
</script>
