<template>
  <div class="bg-white mx-auto p-4 md:p-7 my-5 md:my-8 w-full rounded-lg">
    ...
    <div class="flex flex-wrap gap-5 justify-center mb-10">
      <div
        class="shadow-md bg-white p-4 w-full sm:w-[300px] rounded-lg text-center sm:text-left"
      >
        <p>Current Value</p>
      ...
      <table class="w-full text-left">
        <thead>
          <tr class="border-b text-gray-400">
            ...
            <th class="py-3 px-4">Quantity Remaining</th>
        ...
        <tbody>
          <tr
            v-for="(item, index) in sellBalance"
            :key="index"
            class="border-b"
          >
            ...
            <td class="py-4 px-4">
              <button
                class="border border-[#4aabab] text-[#4aabab] px-3 py-1 rounded-2xl"
              >
                Sell
              </button>
            </td>
...
onMounted(async () => {
  const { data, error } = await getSellRemaining();
  if (data?.success) {
    sellBalance.value = data.data;
  }
});
</script>
